...
function conectarServidor() {
  ...
  ws.onmessage = (evento) => {
    try {
      ...
      if (datos.tipo === 'respuesta') {
        ...
        // Si el mensaje habla del ESP32, prendemos el punto verde
        if (datos.mensaje.includes('ESP32 conectado')) {
          actualizarEstadoEsp32(true);
        }
        ...
        // Si el error es porque se desconectó el ESP32, apagamos el punto
        if (datos.mensaje.includes('ESP32 desconectado')) {
          actualizarEstadoEsp32(false);
        }
      }
  ...
  ws.onclose = () => {
    ...
    // Si se corta la conexión con el servidor, ya no podemos saber si el
    // ESP32 sigue conectado, así que apagamos el punto por las dudas
    actualizarEstadoEsp32(false);
...
/* ── ESTADO DE CONEXIÓN DEL ESP32 ────────────────────────────────
   Prende/apaga el punto verde del header según si el ESP32 está
   conectado al servidor o no. Se llama desde ws.onmessage y desde
   ws.onclose (arriba en este mismo archivo)                      */
function actualizarEstadoEsp32(conectado) {
  const punto = document.getElementById('puntoEsp32');
  const texto = document.getElementById('textoEsp32');
  if (!punto || !texto) return;   // por si esta pantalla no tiene el indicador

  if (conectado) {
    punto.classList.add('conectado');
    texto.textContent = 'Conectado';
  } else {
    punto.classList.remove('conectado');
